...
final class Genesis {
    ...
    static final byte[] generationSignature = Constants.isTestnet ?
            new byte[] {83, 80, -35, 52, 79, 59, 1, 2, -44, -28, -104, -57, 78, 49, -68, 124, 38, -28, -11, 62, 101, -97, 45, 126, -111, 5, -35, -25, -67, -102, -44, -103}:
            new byte[] {63, 86, -90, -102, -82, -16, -69, -40, -124, -44, 48, 1, 98, 68, 86, 69, -57, 44, -8, -100, 93, -89, -60, -46, 72, 102, -33, 43, -27, -46, -74, 36};
    ...
    private static void importAliases(MessageDigest digest) {
        try (InputStreamReader is = new InputStreamReader(new DigestInputStream(
                ClassLoader.getSystemResourceAsStream("data/SEMAR_ALIASES" + (Constants.isTestnet ? "-testnet.json" : ".json")), digest), "UTF-8")) {
            JSONObject aliases = (JSONObject) JSONValue.parseWithException(is);
            Logger.logDebugMessage("Loading aliases");
            int count = 0;
            long aliasId = 1;
            AliasHome ignisAliasHome = ChildChain.SEMAR.getAliasHome();
    ...
    private static void importCurrencies(MessageDigest digest) {
        try (InputStreamReader is = new InputStreamReader(new DigestInputStream(
                ClassLoader.getSystemResourceAsStream("data/SEMAR_CURRENCIES" + (Constants.isTestnet ? "-testnet.json" : ".json")), digest), "UTF-8")) {
