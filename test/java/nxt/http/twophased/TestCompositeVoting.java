...
public class TestCompositeVoting extends BlockchainTest {
    ...
    public void testTwoSecretsApproveAtOnce() {
        ...
        builder.feeNQT(4 * ChildChain.SEMAR.ONE_COIN).param("recipient", BOB.getStrId()).param("amountNQT", amount);
        ...
        //Approved
        Assert.assertEquals(amount, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void testAssetAndCurrency() {
        ...
        Assert.assertEquals(ACTestUtils.PhasingStatus.REJECTED, ACTestUtils.getPhasingStatus(fullHash));
        Assert.assertEquals(0, DAVE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        ...
        generateBlocks(3);
        //Approved
        Assert.assertEquals(100 * ChildChain.SEMAR.ONE_COIN, DAVE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        ...
        Assert.assertEquals(200 * ChildChain.SEMAR.ONE_COIN, DAVE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    ...
    public void testNegatedWhitelist() {
        ...
        //Not yet approved
        Assert.assertEquals(ACTestUtils.PhasingStatus.PENDING, ACTestUtils.getPhasingStatus(fullHash));
        Assert.assertEquals(0, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-100 * ChildChain.SEMAR.ONE_COIN - fee, BOB.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));

        generateBlock();
        //Approved by simply waiting for the finish height
        Assert.assertEquals(100 * ChildChain.SEMAR.ONE_COIN, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        ...
        Assert.assertEquals(ACTestUtils.PhasingStatus.REJECTED, ACTestUtils.getPhasingStatus(fullHash));
        Assert.assertEquals(-fee, DAVE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void xorApprovalNoApproval() {
        ...
        Assert.assertEquals(0, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-fee, BOB.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void testNegatedAssetApproved() {
        ...
        //Approved by simply waiting for the finish height
        Assert.assertEquals(100 * ChildChain.SEMAR.ONE_COIN, CHUCK.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void testNegatedAssetRejected() {
        ...
        // still not rejected
        Assert.assertEquals(0, CHUCK.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-100 * ChildChain.SEMAR.ONE_COIN - fee, DAVE.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));

        generateBlocks(3);

        // Rejected at finish height
        Assert.assertEquals(-fee, DAVE.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void testByPropertyAndNone() {
        ...
        builder = createGenericBuilder(ALICE, BOB, 3 * ChildChain.SEMAR.ONE_COIN);
    ...
    public void testNoEarlyFinish() {
        ...
        APICall.Builder builder = createGenericBuilder(ALICE, BOB, 3 * ChildChain.SEMAR.ONE_COIN);
        builder.param("phasingExpression", "A & N");
    ...
    public void testByTwoProperties() {
        ...
        ACTestUtils.PhasingBuilder builder = createGenericBuilder(ALICE, BOB, 4 * ChildChain.SEMAR.ONE_COIN);
        builder.votingModel(VoteWeighting.VotingModel.COMPOSITE).quorum(1).phasingParam("Expression", "A & B");
    ...
    public void testLinkedTransactionsGrouping() {
        ...
        builder.votingModel(VoteWeighting.VotingModel.COMPOSITE).quorum(1).phasingParam("Expression", "T1 & T2");
        builder.startSubPoll("T1").votingModel(VoteWeighting.VotingModel.TRANSACTION).phasingParam("LinkedTransaction",
                ChildChain.SEMAR.getId() + ":" + fullHash).quorum(1);
        builder.startSubPoll("T2").votingModel(VoteWeighting.VotingModel.TRANSACTION).phasingParam("LinkedTransaction",
                ChildChain.SEMAR.getId() + ":" + fullHash).quorum(1);
    ...
    public void testWhitelistedCoinVoting() {
        ...
        builder.startSubPoll("A2").votingModel(VoteWeighting.VotingModel.COIN).whitelist(DAVE)
                .quorum(DAVE.getInitialChainBalance(ChildChain.SEMAR.getId()) + 1); //A2 cannot be approved
    ...
    private JSONObject createPhasedAssetAndCurrency(Tester recipient, String assetId, String currencyId) {
        long fee = 6 * ChildChain.SEMAR.ONE_COIN;
