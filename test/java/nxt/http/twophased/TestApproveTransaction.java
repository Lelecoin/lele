...
public class TestApproveTransaction extends BlockchainTest {
    ...
    public void validVoteCasting() {
        ...
        apiCall = new APICall.Builder("approveTransaction")
                ...
                .param("phasedTransaction", ChildChain.SEMAR.getId() + ":" + transactionJSON.get("fullHash"))
                .param("feeNQT", ChildChain.SEMAR.ONE_COIN)
                .build();
        ...
        Assert.assertEquals(-50 * ChildChain.SEMAR.ONE_COIN - 2 * ChildChain.SEMAR.ONE_COIN,
                ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(50 * ChildChain.SEMAR.ONE_COIN, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-ChildChain.SEMAR.ONE_COIN, CHUCK.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void invalidVoteCasting() {
        ...
        Assert.assertEquals("ALICE balance: ", -2 * ChildChain.SEMAR.ONE_COIN,
                ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals("BOB balance: ", 0, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals("CHUCK balance: ", 0, CHUCK.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals("DAVE balance: ", 0, DAVE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    ...
    public void sendMoneyPhasedNoVoting() {
        long fee = 2* ChildChain.SEMAR.ONE_COIN;
        JSONObject response = new APICall.Builder("sendMoney").
                param("secretPhrase", ALICE.getSecretPhrase()).
                param("recipient", BOB.getStrId()).
                param("amountNQT", 100 * ChildChain.SEMAR.ONE_COIN).
                param("feeNQT", fee).
                param("phased", "true").
        ...
        // Forger
        Assert.assertEquals(fee, FORGY.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(fee, FORGY.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
        // Sender
        Assert.assertEquals(-fee, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-100 * ChildChain.SEMAR.ONE_COIN - fee, ALICE.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
        // Recipient
        Assert.assertEquals(0, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(0, BOB.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));

        generateBlock();
        // Transaction is applied
        // Sender
        Assert.assertEquals(-100 * ChildChain.SEMAR.ONE_COIN - fee, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-100 * ChildChain.SEMAR.ONE_COIN - fee, ALICE.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
        // Recipient
        Assert.assertEquals(100 * ChildChain.SEMAR.ONE_COIN, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(100 * ChildChain.SEMAR.ONE_COIN, BOB.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void sendMoneyPhasedByTransactionHash() {
        ...
        long fee = 3 * ChildChain.SEMAR.ONE_COIN;
        response = new APICall.Builder("sendMoney").
                ...
                param("phasingLinkedTransaction", ChildChain.SEMAR.getId() + ":" + fullHash).
    ...
    public void sendMoneyPhasedByTransactionHash2of3() {
        ...
        String chainPrefix = ChildChain.SEMAR.getId() + ":";
        long fee = 5 * ChildChain.SEMAR.ONE_COIN;
        response = new APICall.Builder("sendMoney").
                param("secretPhrase", ALICE.getSecretPhrase()).
                param("recipient", BOB.getStrId()).
                param("amountNQT", 100 * ChildChain.SEMAR.ONE_COIN).
    ...
    public void sendMoneyPhasedByTransactionHashNotApplied() {
        ...
        JSONObject response = new APICall.Builder("sendMoney").
                ...
                param("phasingFinishHeight", baseHeight + 2).
                param("phasingVotingModel", 4).
                param("phasingLinkedTransaction", ChildChain.SEMAR.getId() + ":a13bbe67211fea8d59b2621f1e0118bb242dc5000d428a23a8bd47491a05d681"). // this hash does not match any transaction
                param("phasingQuorum", 1).
                build().invoke();
        Logger.logDebugMessage("sendMoney: " + response);
        ...
        // Sender
        Assert.assertEquals(-fee, ALICE.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(-fee, ALICE.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
        // Recipient
        Assert.assertEquals(0, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
        Assert.assertEquals(0, BOB.getChainUnconfirmedBalanceDiff(ChildChain.SEMAR.getId()));
    }
    ...
    public void testInvalidHash() {
        long amount = 100 * ChildChain.SEMAR.ONE_COIN;
        String secret = "abc";
    ...
    public void testApproveTwoTransactionsWithOneSecret() {
        ...
        approveBuilder.param("phasedTransaction", new String[] { ChildChain.SEMAR.getId() + ":" + fullHash1,  ChildChain.SEMAR.getId() + ":" + fullHash2});

        new JSONAssert(approveBuilder.build().invoke()).str("fullHash");

        generateBlock();

        Assert.assertEquals(2 * amount - ChildChain.SEMAR.ONE_COIN, BOB.getChainBalanceDiff(ChildChain.SEMAR.getId()));
    }
