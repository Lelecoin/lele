...
public class AssetExchangeTest extends BlockchainTest {
    ...
    static JSONObject issueAsset(Tester creator, String name) {
        APICall apiCall = new APICall.Builder("issueAsset")
                ...
                .param("feeNQT", 1000 * ChildChain.SEMAR.ONE_COIN)
                .param("deadline", 1440)
                .build();
    ...
    public void ignisDividend() {
        ...
        // Pay dividend in SEMAR, nice and round
        int chainId = ChildChain.SEMAR.getId();
        payDividend(assetId, ALICE, Nxt.getBlockchain().getHeight(), 1000000L, ChildChain.SEMAR, HoldingType.COIN.getCode(), "");
        generateBlock();
        Assert.assertEquals(3 * ChildChain.SEMAR.ONE_COIN, BOB.getChainBalanceDiff(chainId));
        Assert.assertEquals(2 * ChildChain.SEMAR.ONE_COIN, CHUCK.getChainBalanceDiff(chainId));
        Assert.assertEquals(ChildChain.SEMAR.ONE_COIN, DAVE.getChainBalanceDiff(chainId));
    }

    @Test
    public void IDDRDividend() {
        ...
        int chainId = ChildChain.IDDR.getId();
        payDividend(assetId, RIKER, Nxt.getBlockchain().getHeight(), 1L, ChildChain.IDDR, HoldingType.COIN.getCode(), "");
        generateBlock();
        Assert.assertEquals(-555-222-111 - ChildChain.IDDR.ONE_COIN, RIKER.getChainBalanceDiff(chainId));
    ...
    public void assetDividend() {
        ...
        payDividend(assetId, RIKER, Nxt.getBlockchain().getHeight(), 1L, ChildChain.IDDR, HoldingType.ASSET.getCode(), receiverId);
        generateBlock();
    ...
    public void currencyDividend() {
        ...
        payDividend(assetId, ALICE, Nxt.getBlockchain().getHeight(), 1L, ChildChain.IDDR, HoldingType.CURRENCY.getCode(), currencyId);
        generateBlock();
